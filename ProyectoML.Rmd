...
```{r}
ajuste.lasso <- glmnet(x,y,alpha=1,lambda =round(cv.lasso$lambda.1se,3))
coef(lasso.final)[coef(ajuste.lasso)[,1] !=0,]
```
...
```{r}
library(ResourceSelection)
hoslem.test(datos$Enfermedad_Cardiaca_Ataque, fitted(ajuste.lasso), g = 8)
```
...
Prediccion
```{r}
prediccion <- predict(ajuste.lasso,test)
```
