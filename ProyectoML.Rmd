...
# data$Veggies <- as.factor(data$Smoker)
...
```{r}
ajuste_bueno <- glm(Enfermedad_Cardiaca_Ataque ~ Presion_Sanginea_Alta + Colesterol_Alto +  Fumador + Ataque + Diabetes + Salud_General + Salud_Fisica + Dificultad_Andar +  Ingreso, family = "binomial", data=entrenamiento)
...
```{r}
ajuste_nuevo <- glm(Enfermedad_Cardiaca_Ataque ~ Presion_Sanginea_Alta + Colesterol_Alto + 
...
hoslem.test(entrenamiento$Enfermedad_Cardiaca_Ataque, fitted(ajuste), g = 8)
```
